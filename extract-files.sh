...
CAM32="$XTENDED_ROOT"/vendor/"$VENDOR"/"$DEVICE"/proprietary/vendor/lib/hw/camera.qcom.so
patchelf --replace-needed libhidlbase.so libhidlbase-v29.so "$CAM32"

CAM64="$XTENDED_ROOT"/vendor/"$VENDOR"/"$DEVICE"/proprietary/vendor/lib64/hw/camera.qcom.so
